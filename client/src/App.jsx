...
function App() {
 ...
 function makeTempId() {
  ...
  function submitAddTask() {
    ...
    const tempTask = { _id: tempId, title, status: defaultStatus, isLocal: true }
  ...
  api.post('/tasks', { title, status: defaultStatus })
      ...
      .catch(err => {
        ...
        setTasks(prev => prev.map(t => t._id === tempId ? { ...t, syncError: true } : t))
      })
  }
  ...
  function submitEditColumn() {
    ...
    if (!trimmed) return
    setColumnsState(prev => prev.map(c => c.key === editingColumnKey ? { ...c, label: trimmed } : c))
  ...
  api.put(`/tasks/${id}`, updates)
      ...
      .catch(err => {
        ...
        setTasks(prev => prev.map(t => t._id === id ? { ...t, syncing: false, syncError: true } : t))
      })
  }
  ...
  function handleDrop(e, newStatus) {
    ...
    if (task && task.status !== newStatus) {
      setTasks(prev => prev.map(t => t._id === id ? { ...t, status: newStatus, syncing: true } : t))
...
<h1 className="text-2xl font-bold">Kanban Board (Styling)</h1>
   ...
   <main className = 'p-6'>
        ...
        <div className="flex gap-6">
          {columnsState.map((col, idx) => (
            <div
              ...
              className={`${idx > 0 ? 'border-l border-gray-200 pl-6' : ''} column-wrapper`}
            >
    ...
    {showAddColumnModal && (
      ...
      {showEditColumnModal && (
        <Modal title="Edit Column" onClose={() => setShowEditColumnModal(false)}>
          <div className="space-y-4">
            <label className="block">
              <span className="text-sm font-medium">Column Name</span>
              <input value={editColumnName} onChange={e => setEditColumnName(e.target.value)} className="w-full border rounded px-3 py-2 mt-1" />
            </label>
            <div className="flex justify-end">
